...
TXT
    ...
    # Note that some devices need to not have -p specified at all.
    my $pad_opt;
    if ($pcf->{DEPLOYMENT_PADDING_EN} eq 'y') {
        $pad_opt = '-p';
        $pad_opt = '--pad=' . ($fs_size * 1024) if $pcf->{DEPLOYMENT_PADDING_KB};
    }

    my $pagesize_opt = '-s ' . ($pcf->{DEPLOYMENT_PAGESIZE_KB} * 1024);
    ...
    then
        ...
        then
            ...
            then
                ...
                -a $pcf->{SYSCFG_RUNKERNELADDR} -e $pcf->{SYSCFG_RUNKERNELADDR} \\
                -n "Linux Kernel Image" -d $tdir/image.bin.gz $tdir/uImage.bin
            fi
        fi
