...
Patch2		: zaptel-1.4.12.1-m68k-wcfxs-fix.patch
Patch3		: zaptel-1.4.12.1-m68k-build-error-fix.patch
...
make install-modules install-programs ARCH=$LINTARCH DESTDIR=$RPM_BUILD_ROOT/%{pfx} HOSTCC='/usr/bin/gcc' HOSTCFLAGS='-B/usr/bin' HOSTLDFLAGS="-B/usr/bin" HOSTLD=/usr/bin/ld HOSTAR=/usr/bin/ar KSRC=$KSRC_DIR KVERS=$KVER DEB_HOST_GNU_TYPE=$LINTARCH HOSTPATH=$UNSPOOF_PATH
