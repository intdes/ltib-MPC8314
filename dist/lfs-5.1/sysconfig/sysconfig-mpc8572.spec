...
if [ "$SYSCFG_START_PME_DEV" = "y" ]
then
	pme_dev_init=pme_dev_init
fi
...
all_services="mount-proc-sys mdev udev hostname devfsd depmod modules filesystems syslog network inetd portmap dropbear sshd boa smb dhcpd settime qtopia pme_dev_init watchdog gtk2 pango"
all_services_r="pango gtk2 watchdog pme_dev_init qtopia settime dhcpd smb boa sshd dropbear portmap inetd network syslog filesystems modules depmod devfsd hostname udev mdev mount-proc-sys"

cfg_services="mount-proc-sys $mdev $udev hostname $devfsd depmod modules filesystems $syslog $network $inetd $portmap $dropbear $sshd $boa $smb $dhcpd $settime $qtopia $pme_dev_init $watchdog $gtk2 $pango"

cfg_services_r="$pango $gtk2 $watchdog $pme_dev_init $qtopia $settime $dhcpd $smb $boa $sshd $dropbear $portmap $inetd $network $syslog filesystems modules depmod $devfsd hostname $udev $mdev mount-proc-sys"
